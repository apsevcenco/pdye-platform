...
export default function AdminPlatformNda() {
  ...
  const [history, setHistory] = useState<PlatformNdaDocument[]>([]);
  ...
  async function load() {
    ...
    try {
      const [docs, sigs] = await Promise.all([
        platformNdaApi.adminGet(),
        platformNdaApi.adminListSignatures(),
      ]);
  ...
  async function handlePublish() {
    ...
    if (!confirm(
      ...
      `This will replace the current active document. New users will sign this version. ` +
    ...
    try {
      await platformNdaApi.adminPublish({
      ...
      <header className="border-b border-white/10 bg-[#0a1426]/90 backdrop-blur sticky top-0 z-10">
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="ml-3 text-[10px] uppercase tracking-widest text-white/40">Admin · Platform NDA</span>
          </div>
