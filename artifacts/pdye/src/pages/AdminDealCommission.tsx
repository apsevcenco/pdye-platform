...
export default function AdminDealCommission() {
  ...
  async function handleDownloadSig(sig: DealCommissionSignatureRow) {
    ...
    try {
      const blob = await dealCommissionApi.downloadSigned(sig.deal_room_id, sig.side);
      const safeName = sig.signature_name.replace(/[^A-Za-z0-9]+/g, "_").slice(0, 40);
      triggerBlobDownload(blob, `PDYE-Commission-${sig.document_version}-${sig.side}-${safeName}.pdf`);
    } catch (e: any) {
  ...
  async function handlePublish() {
    ...
    setPublishMessage(null);
    try {
      await dealCommissionApi.adminPublish({
                  ...
                  <tbody>
                    {signatures.map(s => (
                      <tr key={s.id} className="border-b border-white/5 last:border-b-0">
                        ...
                        <td className="p-3 font-mono text-[10px] text-white/50" title={s.deal_room_id}>{s.deal_room_id.slice(0, 8)}…</td>
